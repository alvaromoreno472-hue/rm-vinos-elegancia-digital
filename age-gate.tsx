...
import { CATEGORY_IMAGE } from "@/lib/wines";

export function WineCard({ wine }: { wine: Wine }) {
  return (
    <article className="group flex flex-col">
      <Link to="/vinos" className="relative block overflow-hidden bg-wine-deep aspect-[3/4]">
        <img
          src={CATEGORY_IMAGE[wine.category]}
      ...
      <div className="mt-5 flex flex-col text-center">
        <span className="eyebrow">{wine.bodega ?? wine.region}</span>
        <h3 className="mt-2 font-display text-2xl text-wine-deep">{wine.name}</h3>
        {wine.location && <p className="mt-1 text-sm text-muted-foreground">{wine.location}</p>}
